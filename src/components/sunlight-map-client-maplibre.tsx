...
/**
 * MapLibre rewrite of the homepage map client.
 *
 * Duplicate-and-swap strategy: this file starts as a verbatim copy of
 * `sunlight-map-client.tsx` (the Leaflet version still wired to `/`) and is
 * being incrementally converted to use the MapLibre engine + the
 * `components/maplibre-preview/*` modules (sunlight custom layer, places
 * source, place card, LRU tile cache, anti-aliased + outlined shader, etc.).
 *
 * The Leaflet client stays untouched in the original file until this rewrite
 * is fully functional; at that point `app/page.tsx` flips its import to
 * `SunlightMapClientMapLibre` and the old file is archived.
 *
 * Progress is tracked commit-by-commit on `feature/maplibre-migration`.
 */
...
export interface SunlightMapClientMapLibreProps {
...
export function SunlightMapClientMapLibre({ forceCacheOnly }: SunlightMapClientMapLibreProps) {
