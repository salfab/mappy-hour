...
async function main() {
  ...
  console.log(`[terrain-ch-nyon] Fetching STAC items for bbox ${NYON_LOCAL_BBOX.join(",")}...`);
  ...
  console.log(
    `[terrain-ch-nyon] Found ${fetched.items.length} items across ${fetched.pagesFetched} STAC pages.`,
  );
  ...
  const downloadSummary = await downloadStacAssets({
    ...
    assetSelector: (_assetKey, asset) => asset.href.endsWith("_2_2056_5728.tif"),
  });
  ...
  console.log(`[terrain-ch-nyon] Manifest written to ${manifestPath}`);
  console.log(
    `[terrain-ch-nyon] Downloaded ${downloadSummary.filesDownloaded} files (skipped: ${downloadSummary.filesSkipped}).`,
  );
}

main().catch((error) => {
  console.error(
    `[terrain-ch-nyon] Failed: ${error instanceof Error ? error.message : "Unknown error"}`,
  );
  process.exitCode = 1;
});
