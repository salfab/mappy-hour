...
export async function runForRegion(
  ...
  const fetched = await fetchStacItems({ collection: COLLECTION, bbox: [...bbox] as number[], maxItems, limit: 200 });
  ...
  const downloadSummary = await downloadStacAssets({
    items: fetched.items, destinationRoot, dryRun, overwrite,
    assetSelector: (assetKey, asset) => assetKey.endsWith(".tif") || asset.href.endsWith(".tif") || asset.type?.includes("tiff") === true,
  });
  const manifest = { generatedAt: new Date().toISOString(), collection: COLLECTION, region, bbox, dryRun, overwrite, maxItems: maxItems ?? null, itemsFetched: fetched.items.length, pagesFetched: fetched.pagesFetched, downloadSummary };
  if (!dryRun) {
  ...
  return { region, itemsFetched: fetched.items.length, filesDownloaded: downloadSummary.filesDownloaded, filesSkipped: downloadSummary.filesSkipped, manifestPath };
}

function parseRegion(argv: string[]): PrecomputedRegionName {
  for (const arg of argv) { if (arg.startsWith("--region=")) return arg.slice(9) as PrecomputedRegionName; }
  console.error(`Usage: --region=${Object.keys(REGIONS).join("|")}`); process.exit(1);
}

async function main() {
  const region = parseRegion(process.argv.slice(2));
  if (!REGIONS[region]) { console.error(`Unknown region: ${region}`); process.exit(1); }
