...
import { LAUSANNE_HORIZON_BBOX } from "../../src/lib/config/lausanne";
...
async function main() {
  ...
  const manifestPath = path.join(destinationRoot, "manifest-lausanne-horizon.json");

  const allTiles = listTilesForBbox(LAUSANNE_HORIZON_BBOX);
  ...
  console.log(
    `[horizon-dem] Target tiles: ${tiles.length} (bbox: ${LAUSANNE_HORIZON_BBOX.join(",")}).`,
  );
  ...
  console.log(`[horizon-dem] Manifest written to ${manifestPath}`);
  console.log(
    `[horizon-dem] Downloaded ${downloaded}, skipped ${skipped}, not-found ${notFound}.`,
  );
}
