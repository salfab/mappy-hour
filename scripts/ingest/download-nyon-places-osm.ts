...
function buildOverpassQuery(): string {
  const [minLon, minLat, maxLon, maxLat] = NYON_LOCAL_BBOX;
...
async function main() {
  ...
  await ensureParentDirectory(RAW_NYON_PLACES_PATH);
  await fs.writeFile(RAW_NYON_PLACES_PATH, JSON.stringify(rawPayload, null, 2), "utf8");

  await ensureParentDirectory(PROCESSED_NYON_PLACES_PATH);
  await fs.writeFile(
    PROCESSED_NYON_PLACES_PATH,
  ...
  console.log(`[places-nyon] Wrote ${normalizedPlaces.length} places to ${PROCESSED_NYON_PLACES_PATH}.`);
}

main().catch((error) => {
  console.error(
    `[places-nyon] Failed: ${error instanceof Error ? error.message : "Unknown error"}`,
  );
  process.exitCode = 1;
});
