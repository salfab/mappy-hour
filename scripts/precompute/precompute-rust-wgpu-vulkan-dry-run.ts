...
function toValueSummary(
  ...
  return {
    ...
    frames: artifact.frames.map((frame) => ({
      ...
      sunMaskBase64: Buffer.from(frame.sunMask).toString("base64"),
      sunMaskNoVegetationBase64: Buffer.from(frame.sunMaskNoVegetation).toString("base64"),
      terrainBlockedMaskBase64: Buffer.from(frame.terrainBlockedMask).toString("base64"),
      buildingsBlockedMaskBase64: Buffer.from(frame.buildingsBlockedMask).toString("base64"),
      vegetationBlockedMaskBase64: Buffer.from(frame.vegetationBlockedMask).toString("base64"),
    })),
