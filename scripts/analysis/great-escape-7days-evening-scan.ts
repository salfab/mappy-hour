...
const SCAN_START_LOCAL_TIME = "15:00";
...
function sunnyCountFromStates(states: PointState[]): number {
  return states.reduce((count, point) => count + (point.isSunny ? 1 : 0), 0);
}

function compareDayToDayAtSlot(
  previousStates: PointState[],
  currentStates: PointState[],
) {
  const previousById = new Map(previousStates.map((point) => [point.id, point.isSunny] as const));
  const currentById = new Map(currentStates.map((point) => [point.id, point.isSunny] as const));
...
async function main() {
  ...
  for (let i = 0; i < DAYS; i += 1) {
    ...
    console.log(
      `[great-escape-evening] ${date}: ${slots.length} slots (${SCAN_START_LOCAL_TIME} -> ${endSlot})`,
    );

    const slotRows: Array<{ localTime: string; sunnyCount: number; sunnyRatioPct: number }> = [];
    for (const localTime of slots) {
      const states = evaluateStates(prepared, date, localTime);
      const sunnyCount = sunnyCountFromStates(states);
  ...
  const latestSlotWithAnySun = [...aggregateBySlot.entries()]
    ...
    .filter((row) => row.avgSunnyCount > 0)
    .sort((a, b) => (a.localTime < b.localTime ? 1 : -1))[0] ?? null;

  const dayToDayAtLatestAnySun = [];
  if (latestSlotWithAnySun) {
    let previous: { date: string; states: PointState[] } | null = null;
    for (let i = 0; i < DAYS; i += 1) {
      const date = addDays(START_DATE, i);
      const states = evaluateStates(prepared, date, latestSlotWithAnySun.localTime);
      const sunnyCount = sunnyCountFromStates(states);
      if (previous) {
        dayToDayAtLatestAnySun.push({
          fromDate: previous.date,
          toDate: date,
          localTime: latestSlotWithAnySun.localTime,
          sunnyCountDelta:
            sunnyCount - sunnyCountFromStates(previous.states),
          ...compareDayToDayAtSlot(previous.states, states),
        });
      }
      previous = { date, states };
    }
  }
  ...
  const report = {
    ...
    latestSlotWithAnySun,
    dayToDayAtLatestAnySun,
    days: dayResults,
  };
  ...
  const outputPath = path.join(
    ...
    "progress",
    "analysis",
    "great-escape-7days-evening-scan-20260308.json",
  );
  await fs.mkdir(path.dirname(outputPath), { recursive: true });
  await fs.writeFile(outputPath, JSON.stringify(report, null, 2), "utf8");
  console.log(`[great-escape-evening] wrote ${outputPath}`);
}

void main().catch((error) => {
  console.error(
    `[great-escape-evening] Failed: ${
