...
async function check(tileId: string, label: string, centerE: number, centerN: number) {
  ...
  const minutes = ["07:30","09:00","10:30","12:00","14:00","16:00","18:00"];
...
async function main() {
  // Montriond W (school), Montriond E (includes Villard), Royal Savoy tile
  await check("e2536750_n1152000_s250", "Montriond W (école)", 2536875, 1152125);
  await check("e2537000_n1152000_s250", "Montriond E (Villard)", 2537125, 1152125);
  await check("e2537500_n1151750_s250", "Royal Savoy zone", 2537625, 1151875);
}
